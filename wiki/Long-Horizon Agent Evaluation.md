...
- 偏好冲突 workflow：[[MAESTRO_ Adapting GUIs and Guiding Navigation with User Preferences in Conversational Agents with GUIs]] 强调早期选择约束后续选项时的回退导航。
- 复杂单步动作诊断：[[Uncertainty-Aware GUI Agent_ Adaptive Perception through Component Recommendation and Human-in-the-Loop Refinement]] 的 ComplexAction 不评完整长程任务，但有助于定位长程失败中的单步感知瓶颈。
...
MAESTRO 补充了另一类长程问题：多步骤任务中的偏好冲突不一定表现为操作失败，而是表现为后续选项无法满足早先或后续表达的软偏好。RecAgent 的 ComplexAction 则把长程失败拆回复杂页面中的单步定位问题。
...
- 单步复杂动作 benchmark 和端到端长程 benchmark 应如何组合，才能定位失败来源？
