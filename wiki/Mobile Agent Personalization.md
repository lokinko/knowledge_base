...
- 对话式 GUI 偏好记忆：[[MAESTRO_ Adapting GUIs and Guiding Navigation with User Preferences in Conversational Agents with GUIs]] 将多轮偏好转化为 GUI 原位适配和 workflow 回退建议。
...
- MAESTRO 进一步显示，偏好不只是推荐排序依据，也可以成为界面 filter、sort、highlight 和回退导航的控制信号。
...
- 与 [[GUI Agent]]：偏好驱动的 GUI 适配要求 agent 同时理解用户语言和当前界面约束。
